 ...
 * {@link DateTimeBox.DefaultFormat} when the text does not represent a date that
 * can be parsed</dd>
 * </dl>
 * 
 * <p>
...
public class DateTimeBox extends Composite implements HasValue<Date>,
		IsEditor<LeafValueEditor<Date>>, Multilingual {
	/**
	 ...
	 * to the {@link DateTimeBox}. The style will be removed when either a
	 * successful {@link #parse(DateTimeBox,String, boolean)} is called or
	 * {@link #format(DateTimeBox,Date)} is called.
	 * <p>
	 * Use a different {@link DateTimeBox.Format} instance to change that behavior.
	 * </p>
	 */
...
//			.create(DefaultFormat.class);
	...
	/**
	 * Create a date box with a new {@link DatePicker}.<br>
	 * Time selector is hidden. Use <code>DateTimeBox(int intervalsPerHour)</code>
	 * to enable time selector.
	 */
	public DateTimeBox() {
		this(0);
	}

	public DateTimeBox(int intervalsPerHour) {
		this(new DatePicker(intervalsPerHour), null, new DateTimeBox.DefaultFormat(
				DateTimeFormat.getFormat(intervalsPerHour > 0 ? "dd/MM/yyyy HH:mm"
						: "dd/MM/yyyy")));
		getTextBox().setReadOnly(intervalsPerHour > 0);
	}
...
//		BaseEntryPoint.multilingual.add(this);
	 ...
	 * {@link DateTimeBox}. If this {@link DateTimeBox} is not empty, the contents of
