...
import com.tooooolazy.gwt.flexlayout.client.DynamicFlexLayout;
...
public class DatePicker extends Composite implements
	...
	static class StandardCss {
		...
		public String timeSelector() {
			return wrap("TimeSelector");
		}
		public String timeSelectorMins() {
			return wrap("TimeSelectorMins");
		}
		public String timeSelectorHours() {
			return wrap("TimeSelectorHours");
		}
	 ...
	 */
	public DatePicker() {
		this(new DefaultTimeSelector(), new DefaultMonthSelector(),
				new DefaultCalendarView(), null, false);
	}
	public DatePicker(int intervalsPerHour) {
		this(new DefaultTimeSelector(intervalsPerHour),
				new DefaultMonthSelector(),
				new DefaultCalendarView(), null, true);
	}
	 ...
	 */
//	protected boolean editTime;
	protected DatePicker(TimeSelector timeSelector, MonthSelector monthSelector,
			CalendarView view, CalendarModel model, boolean editTime) {
...
//		this.editTime = editTime;
		...
		if (timeSelector.getIntervalsPerHour() > 0) {
			this.timeSelector = timeSelector;
			timeSelector.setDatePicker(this);
		} else
	...
	public void setTitles() {
		timeSelector.setTitles();
		view.setTitles();
	}
	@Override
	public void addToHandler() {
//		BaseEntryPoint.multilingual.add(this);
	}
	@Override
	public void removeFromHandler() {
//		BaseEntryPoint.multilingual.remove(this);
	}
	@Override
	public String getMessage(String key) {
		return key;
	...
	public HandlerRegistration addShowRangeHandlerAndFire(
			ShowRangeHandler<Date> handler) {
		ShowRangeEvent<Date> event = new ShowRangeEvent<Date>(getView()
				.getFirstDate(), getView().getLastDate()) {
		};
	...
	public boolean isDateVisible(Date date) {
		...
		return (date != null && (CalendarUtil.isSameDate(first, date)
				|| CalendarUtil.isSameDate(last, date) || (first.before(date) && last
				.after(date))));
	}
	...
	public void setCurrentMonth(Date month) {
		if (getTimeSelector() != null) {
			CalendarUtil.normalizeTime(month, 60/getIntervalsPerHour());
		}
	...
	public final void setValue(Date newValue, boolean fireEvents) {
		setValue(newValue, fireEvents, false);
//		Date oldValue = value;
//
//		if (oldValue != null) {
...
//		}
	...
	public final void setValue(Date newValue, boolean fireEvents, boolean allowSame) {
		Date oldValue = value;
		...
		value = CalendarUtil.copyDate(newValue);
		if (value != null && timeSelector != null) {
			CalendarUtil.normalizeTime(value, 60/getIntervalsPerHour());
			CalendarUtil.normalizeTime(newValue, 60/getIntervalsPerHour());
		}
		if (value != null) {
			addStyleToDates(css().dayIsValue(), value);
		}
		if (fireEvents) {
			DateChangeEvent.fireIfNotEqualDates(this, oldValue, newValue, allowSame);
		}
	}

	public int getIntervalsPerHour() {
		return timeSelector.getIntervalsPerHour();
	}
	/**
	...
	protected final TimeSelector getTimeSelector() {
		return timeSelector;
	}
	...
	protected void setup() {
		...
		if (getTimeSelector() != null)
			panel.add(this.getTimeSelector());
	}
